...
//首页 DONE
myControllers.controller('IndexCtrl', function ($scope, $location, $routeParams,$swipe, StockService, ADBannerService) {
	X.loading.show();
	var agent_code = $routeParams['agent_code'] || '';

	var storage = window.localStorage,  agentStr = storage.getItem('agentCode') || '';
	/*if (agentStr == '' || agentStr != agent_code) {
		// if(agentStr01 != ''){storage.removeItem('agentCode01')}
		if(agent_code == agent_codes[0]){storage.setItem('agentCode', agent_code);}
	}
	if (agentStr01 == '' || agentStr01 != agent_code) {
		if(agent_code == agent_codes[1]){
			// if(agentStr != ''){storage.removeItem('agentCode')}
			storage.setItem('agentCode01', agent_code);
		}
	}*/
	if(agent_code == ''){storage.removeItem('agentCode')}
	if(agentStr == '' || agentStr != agent_code){
...
myControllers.controller('DiscoverCtrl', function ($scope, $sce, NoticeService,SystemService) {
	...
	if(agentCode != ''){$('.activity-nav a')[0].style.width = '50%';}
	$scope.indexUrl = SystemService.agentIndex();
